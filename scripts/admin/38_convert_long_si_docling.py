...
def title_from_markdown(markdown: str, fallback: str) -> str:
    for raw in markdown.splitlines()[:80]:
        value = compact(re.sub(r"<[^>]+>", "", raw).lstrip("#").strip())
        if 20 <= len(value) <= 400 and value.lower() not in {"abstract", "introduction", "supporting information"}:
            return value
    return fallback
...
def main() -> int:
    ...
    run_id = "docling_long_si_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    ...
    for index, item in enumerate(candidates, start=1):
        ...
        result = {
            "item_id": item["item_id"], "record": item["record"],
            "category": item["category"], "role": item["role"],
            "source_relative_path": item["source_relative_path"],
            "source_sha256": item["source_sha256"], "extension": item["extension"],
            "status": "failed",
        }
        try:
            if sha256_file(source) != item["source_sha256"]:
                raise RuntimeError("Source hash changed after preflight")
            ...
            warnings = []
            if metrics["full_text_characters"] < 1000:
                warnings.append("docling_text_below_1000_characters")
            payload = {
                "schema_version": 1,
                "source": {"relative_path": item["source_relative_path"], "sha256": item["source_sha256"], "source_modified": False},
                "document": {"title": title_from_markdown(markdown, item["record"] + " supplementary material"), "markdown": markdown},
                "metrics": metrics,
            ...
            result.update({"status": "parsed", "warnings": warnings, "metrics": metrics, "output_relative_path": str(output_path.relative_to(PROJECT_ROOT)), "output_sha256": sha256_file(output_path)})
        except Exception as exc:
            result["error"] = repr(exc)
            result["traceback"] = traceback.format_exc()
        result["duration_seconds"] = round(time.perf_counter() - started, 3)
        ledger.append(result)
        print(f"{index}/{len(candidates)} {result['item_id']} {result['extension']} {result['status']} {result['duration_seconds']}s", flush=True)
    ...
    summary = {"schema_version": 1, "run_id": run_id, "preflight_run": preflight_id, "selected_long_si_count": len(candidates), "status_counts": dict(status_counts), "warning_counts": dict(warning_counts), "source_files_modified": False, "indexing_status": "not_indexed"}
    write_json(run_root / "ledger.json", ledger)
    write_json(run_root / "summary.json", summary)
    (BASE / "latest_docling_long_si_run.txt").write_text(run_id + "\n", encoding="ascii")
